...
local get = ya.sync(function(st, url)
	return st.last == url and st.level
end)
...
local function peek(_, job)
	...
	local level = ya.clamp(-20, job.new_level or get(Url(url)) or tonumber(job.args[1]) or 0, 20)
	local sync = function()
...
local function entry(self, job)
	...
	local new = ya.clamp(-20, st.level + motion, 20)
